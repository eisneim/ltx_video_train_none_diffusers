...
class Captioner():
    ...
    def get_caption(self, frames, size=(640, 320), frame_skip=1, num_predict=80):
        self.client = ollama.Client()
...
class VideoFramesDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        prompt_prefix = "worst quality, inconsistent motion, blurry, jittery, distorted",
        crop_to_fit = False,
    ):
        super().__init__()
    ...
    def cache_frames(self, to_latent, to_caption, to_embedding, device="cuda"):
        ...
        for ii, vid in enumerate(tqdm(self.videos)):
            ...
            if self.crop_to_fit:
                ...
                width = info.width
                height = info.height
                assert width >= self.width, f"视频原始宽{width}应该大于等于目标宽度{self.width}"
                assert height >= self.height, f"视频原始高{height}应该大于等于目标高度{self.height}"
            ...
            if self.crop_to_fit:
                startx = max(0, (width - self.width) // 2)
                starty = max(0, (height - self.height) // 2)
                video_frames = video_frames[:, starty:starty+self.height, startx:startx+self.width]
                assert video_frames.shape[1] == self.height
                assert video_frames.shape[2] == self.width
            ...
            for idx in range(iters):
                ...
                # only cap the first frame of the negative samples
                caption = self.prompt_prefix + " " + to_caption(frames[:1])
                ic(caption)
...
if __name__ == "__main__":
    ...
    # video_dir = [
    #     "/media/eisneim/teli-disk/ltx_train_data/3dgs",
    #     "/media/eisneim/teli-disk/ltx_train_data/game_p9",
    #     "/media/eisneim/teli-disk/ltx_train_data/game_p10",
    #     "/media/eisneim/teli-disk/ltx_train_data/game_p11",
    #     "/media/eisneim/teli-disk/ltx_train_data/game_p12",
    #     "/media/eisneim/teli-disk/ltx_train_data/game_p13",
    # ]
    # cache_dir = "/media/eisneim/4T/ltx_data_57x1280x704/all_video"

    video_dir = "/media/eisneim/teli-disk/ltx_train_data/negative_samples"
    cache_dir = "/media/eisneim/4T/ltx_0.9.5/57x1280x704/negative"
    ...
    vae = load_latent_models()["vae"]
    # diffuser 0.32 -> 0.33.0.dev0 cause this issue
    vae.timestep_scale_multiplier = nn.Parameter(torch.tensor(1000.0, dtype=dtype))
    vae = vae.to(device)
